...
col_names = ['x', 'y', 'month', 'day', 'FFMC', 'DMC', 'DC', 'ISI', 'temp', 'RH', 'wind','area']

# load dataset
fires = pd.read_csv("forestfires_cleaned_norain.csv")
fires.columns=col_names

#bin area
bins = [0,3,40,1100] #Yes/No bins
labels = [0,1,2]
fires.area = pd.cut(fires['area'], include_lowest=True, bins=bins, labels=labels)

# Normalizing data to 10
...
feature_cols = ['x', 'y', 'month', 'day', 'FFMC', 'DMC', 'DC', 'ISI', 'temp', 'RH', 'wind']
X = fires[feature_cols] # Features
a = fires.area # Target variable

# Split dataset into training set and test set
X_train, X_test, a_train, a_test = train_test_split(X, a, test_size=0.2, random_state=1) # 70% training and 30% test
...
graph.write_png('DT_SmlMedLrg_.2.png')
...
cv_scores = cross_val_score(knn_cv, X, a, cv=5)
